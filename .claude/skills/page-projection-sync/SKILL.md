...
description: 在 formal relation ledger 更新后，把最新 ledger 投影同步回对象页：更新 `Formal relations`、强一致 frontmatter 和模板化人类关系区块。仅当编排型 skill `process-paper` 已将当前任务推进到 page projection 阶段，或用户明确要求只刷新对象页 projection、同步 `parent_methods` / `child_methods` 等强一致字段、或批量刷新页面中的关系区块时，才应使用本 skill；不要把它当作“处理论文”完整请求的默认入口。
---
...
## 链路位置
本 skill 是单篇论文编译链中的 page projection 阶段，默认前置为 `relation-reconciliation`。
默认由编排型 skill `process-paper` 在 formal ledger 对齐后调用；当用户明确要求只刷新对象页 projection 时，也可直接使用本 skill。
...
字段约定：
- `projected_pages`：本轮完成 formal projection 的页面清单；每项至少包含 `path` 与 `updated_sections`。
- 不再使用 `synced_pages` 作为主交接字段名。
...
- 同步完成后，必须再交给 index-sync、lint、ontology-semantic-review 与 serving-governance-review。
