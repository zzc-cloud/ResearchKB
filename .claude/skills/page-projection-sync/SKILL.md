...
- `references_method` 的首批规则：若 ledger 中存在 `source_paper_path` / `target_paper_path`，则对象页 `Formal relations` 继续以 `source_paper_path` / `target_paper_path` 形式投影为 path metadata，不把它们升级成新的可点击 paper 邻接。
