...
## 在以下场景使用
- 一批 Paper / Method / Concept / Task / Scenario / Benchmark / Evidence 页面已经迁移到 serving-layer 模型。
...
3. **问答可遍历性**
   ...
   - formal neighbor 是否仍然停留在合格的默认 serving surface 上，而不是仅仅“存在即可解析”？
...
- 若当前页结构完整、formal relation 完整、邻居可解析，但关键邻居仍主要是 semantic stub / `partial` 页，应判为 `needs_fixes`，而不是直接视为 `pass`。
