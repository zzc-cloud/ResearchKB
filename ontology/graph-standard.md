...
- `references_method`：`[[Method]] --references_method--> [[Method]]`；表示方法将另一方法作为关键比较对象、借鉴路线或方法参照。该关系与 `based_on` 一起构成方法图谱的核心邻接关系：前者表达参照，后者表达谱系；`references_method` 不驱动 `parent_methods` / `child_methods`。`ontology/relations/references_method.md` 中的每条实例边除 `source_path` / `target_path` 外，还必须记录 `source_paper_path` 与 `target_paper_path`，分别锚定该方法参照关系的抽取来源论文与 target method 对应的代表 / 参考论文；这两个字段属于 paper-level provenance，不改变其 `Method -> Method` formal relation 语义。若一条 `references_method` 实例边同时声明 `source_paper_path` 与 `target_paper_path`，则 formal ledger 中必须存在对应 `cites`；否则该边不得视为 fully valid formal edge。
...
- 正文模板化关系区块必须对 `## Formal relations` 做主题化摘要覆盖，且语义不弱于 formal projection；读者不应必须逐条回读 `Formal relations` 才能知道该页与哪些对象发生了什么类型的核心正式关系。
- 这种摘要覆盖允许按主题聚合，但不得把 formal relation 的关键区分压扁到不可恢复。
...
- 对象页 `Formal relations` 不是 relation ledger child fields 的逐字复制，但必须覆盖 formal relation instance 的全部 serving-necessary attributes。
- 当 relation ledger 存在 relation-specific 必要属性时，对象页必须追加对象页友好的对应字段；例如 `references_method` 的 `source_paper_path → source_paper_path`、`target_paper_path → target_paper_path`，它们作为 path metadata 暴露，但不升级为新的可点击 paper 邻接。
