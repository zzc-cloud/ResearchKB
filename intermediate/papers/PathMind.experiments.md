...
## 1. 数据集与指标
> 本节支撑 [[PathMind]] 在 [[WebQSP]] 与 [[CWQ]] 上的 benchmark 绑定与实验结果解释。
...
## 2. 实现设置
> 本节支撑 [[PathMind]] 的实验可复现性描述，以及 [[WebQSP]] / [[CWQ]] 上的设置差异说明。
...
## 3. 总体结果（RQ1）
> 本节支撑 [[PathMind]] 相对 [[EPERM]]、[[GCR]]、[[RoG]]、[[ToG]] 与 [[Gnn-rag - Graph neural retrieval for efficient large language model reasoning on knowledge graphs|GNN-RAG]] 的总体表现对比。
### PathMind 结果
| Dataset | Hits@1 | F1 |
|---|---:|---:|
| WebQSP | 0.895 | 0.728 |
...
| [[Gnn-rag - Graph neural retrieval for efficient large language model reasoning on knowledge graphs|GNN-RAG]] | 0.864 | 0.690 | 0.679 | 0.591 |
...
- [[CWQ]] 上，[[PathMind]] 相比 [[Gnn-rag - Graph neural retrieval for efficient large language model reasoning on knowledge graphs|GNN-RAG]] 提升更明显，说明在复杂多跳任务上优势更突出。
- 相比纯 LLM backbone（如 Llama3.1-8B、Qwen2-7B），[[PathMind]] 的结构化路径利用显著提升性能。

## 4. 消融实验（RQ2）
> 本节支撑 [[路径优先化]] 与训练对齐机制在 [[PathMind]] 中的必要性判断。
...
## 6. 泛化与可扩展性（RQ3）
> 本节支撑 [[PathMind]] 在不同 backbone 与更复杂任务条件下的泛化能力判断。
### 跨 backbone 泛化
...
## 7. 效率结果
> 本节支撑 [[PathMind]] 相对 [[RoG]]、[[GCR]]、[[ToG]] 与 [[Gnn-rag - Graph neural retrieval for efficient large language model reasoning on knowledge graphs|GNN-RAG]] 等方法的效率比较，以及 [[evidence_index]] 中的实验回链。
...
| [[Gnn-rag - Graph neural retrieval for efficient large language model reasoning on knowledge graphs|GNN-RAG]] | 0.864 | 1.52 | 1 | 414 |
